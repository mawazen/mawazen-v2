...
- الخصم: ${caseData.opposingParty || "غير محدد"}`,
        ...
        let snippets: Awaited<ReturnType<typeof retrieveLegalSnippets>> = [];
        try {
          snippets = await retrieveLegalSnippets({ query: input.message });
        ...
        if (snippets.length > 0 && wantsVerbatimArticleText) {
          messages.push({
            role: "system",
            content:
              "قاعدة إلزامية: إذا طلب المستخدم (نص المادة رقم X) فاطبع نص المادة حرفياً فقط كما ورد داخل المقتطفات (بدون أي شرح أو تفصيل أو إعادة صياغة). لا تضف مقدمات ولا تعليقات.",
          });
        }
